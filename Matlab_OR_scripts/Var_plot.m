function Var_plot(orientations,ebsd_Phase1,ebsd_Phase2)
...
try 
    ...
    disp('No predefined_OR chosen. As default set Greninger-Troiano OR.');
...
figure('name',['filtered child orientations by a threshold of  including Variants ', num2str(threshold)]);
plotPDF(new_ori_mar_filtered_angles,ipfKey_Mar.orientation2color(new_ori_mar_filtered_angles),hChild,'all','MarkerSize',markerSize) % plot filtered martensite on the PF
hold on
plotPDF(new_ori_mar_filtered_angles(plotted_ori),ipfKey_Mar.orientation2color(new_ori_mar_filtered_angles(plotted_ori)),hChild,'all','MarkerSize',markerSize) % plot filtered martensite on the PF
for i=plotted_ori
    annotate(new_ori_mar_filtered_angles(i),...
    'marker','s','MarkerSize',6,'MarkerFaceColor','r',...
    'label',['V',num2str(stor(i,2))],'color','b', 'FontSize',24)
end